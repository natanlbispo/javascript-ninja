...
*/
console.log( '\nJust Numbers:' );
let justNumbers = numberObjects.map(it => {
    return it.number;
});
...
let justMod2Or3 = justNumbers.filter(it => {
    return it % 2 === 0 || it %3 === 0 ;  
});
...
*/
console.log( '\nOperation:' );
let operation = justMod2Or3.reduce((it, id)=>{
    return (it+1)*id;
}, 0);
...
*/
console.log( '\nOperation 2:' );
let operation2 = justMod2Or3.reduceRight((it, id)=>{
    return (it+1)*id;
}, 0);
...
let name = ['Tes','te'];
let linguaDoP = name.reduce((v1, v2)=>{
    return (v1 + 'p') + v2 ;
}, '');
...
let lastNumber2 = numberObjects.lastIndexOf( {number: 2}, 2 ) > -1;
...
let isItAnArray = Array.isArray(justMod2Or3);
if( justMod2Or3 ) {
